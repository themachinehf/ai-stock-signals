...
<script>
async function fetchData(path){try{return await fetch('/api'+path).then(r=>r.json())}catch(e){return null}}
async function refreshData(){document.getElementById('app').innerHTML='<div class="loading">OBSERVING...</div>';const s=await fetchData('/status');const a=await fetchData('/analysis');const g=await fetchData('/signals');render(s,a,g)}
function render(s,a,g){document.getElementById('ts').textContent=new Date().toLocaleString();const m=s?.market||{BTC:{price:69443,change:-1.41},ETH:{price:2096,change:1.80},SOL:{price:87.71,change:0.39}};let h='';Object.entries(m).forEach((e=>{if(e[1]){const m=e[1],c=m.change>=0?'change-positive':'change-negative';h+='<div class="coin-row"><span class="coin-symbol">'+e[0]+'</span><span class="coin-price">$'+m.price.toLocaleString()+'</span><span class="coin-change '+c+'">'+(m.change>=0?'+':'')+m.change.toFixed(2)+'%</span></div>'}}));let i='';if(g?.signals){g.signals.forEach((e=>{i+='<div class="coin-row"><span class="coin-symbol">'+e.symbol+'</span><span class="coin-change">'+(e.action||'NEUTRAL')+'</span></div>'}))}document.getElementById('app').innerHTML='<div class="grid"><div class="card"><div class="card-header"><span class="card-title">Market</span><span class="card-badge">'+(s?.status?.market_sentiment||'NEUTRAL')+'</span></div>'+h+'</div><div class="card"><div class="card-header"><span class="card-title">Analysis</span><span class="card-badge">'+(a?.summary?.dominant_trend||'---')+'</span></div><div style="padding:0.75rem 0;border-bottom:1px solid #2a2a3a"><div style="color:var(--text-secondary);font-size:0.75rem">PHASE</div><div>'+(a?.summary?.market_phase||'---')+'</div></div><div style="padding:0.75rem 0"><div style="color:var(--text-secondary);font-size:0.75rem">RISK</div><div>'+(a?.risk?.overall||'---')+'</div></div></div><div class="card"><div class="card-header"><span class="card-title">Signals</span><span class="card-badge">'+(g?.total_signals||0)+' ACTIVE</span></div>'+(i||'<p style="color:var(--text-secondary)">NO SIGNALS</p>')+'</div></div>'}setInterval(refreshData,60000);refreshData();
</script>
</body>
</html>`;
